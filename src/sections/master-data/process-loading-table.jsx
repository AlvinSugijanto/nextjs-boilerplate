...
import { processLoadingSchema } from "./schema-validation";
...
const ProcessLoadingTable = () => {
  ...
  const methods = useForm({
    resolver: yupResolver(processLoadingSchema),
  ...
  const dialogTitle = isEditMode
    ? "Edit Process Loading"
    : "Add New Process Loading";
  const dialogDescription = isEditMode
    ? "Update the process loading details below."
    : "Fill in the details to add a new process loading.";

  // ====== API Calls ======
  const fetchData = useCallback(async () => {
    loadingFetch.onTrue();
    try {
      const { data } = await axios.get("/api/collection/process_loading", {
        headers: { type: "getfulllist" },
      });
      setData(data);
    } catch (error) {
      console.error("Error fetching Process Loading data:", error);
    } finally {
  ...
  const onSubmit = handleSubmit(async (values) => {
    ...
    try {
      if (isEditMode) {
        // Update existing Process Loading
        const { data: res } = await axios.put(
          `/api/collection/process_loading/${selectedData.id}`,
          values
        );
        setData((prevData) =>
          prevData.map((item) => (item.id === selectedData.id ? res : item))
        );
        toast.success("ProcessLoading updated successfully!");
      } else {
        // Create new processLoading
        const { data: res } = await axios.post(
          "/api/collection/process_loading",
          values
        );
        setData((prevData) => [...prevData, res]);
        toast.success("ProcessLoading created successfully!");
      }
      handleCloseDrawer();
    } catch (error) {
      console.error("Error submitting Process Loading:", error);
      ...
      if (error.response?.data?.data) {
        ...
        toast.error(
          error.response?.data?.message ||
            "Failed to save processLoading. Please try again."
        );
      }
    } finally {
  ...
  const handleDelete = async () => {
    ...
    try {
      await axios.delete(`/api/collection/process_loading/${selectedData.id}`);
      setData((prevData) =>
      ...
      console.error("Error deleting Process Loading:", error);
    } finally {
    ...
    <>
      <Card className="p-4">
        ...
        <div className="flex items-center justify-between mb-4">
          <TypographyLarge>Process Loading</TypographyLarge>
                  ...
                  <div className="flex-1 overflow-auto py-4">
                    <div className="grid gap-6 px-4">
                      <RHFTextField
                        ...
                        placeholder="Enter process loading"
                      />
                    </div>
                  </div>
      ...
      <ConfirmDialog
        ...
        title={`Delete Process Loading "${selectedData?.name}"?`}
...
export default ProcessLoadingTable;
