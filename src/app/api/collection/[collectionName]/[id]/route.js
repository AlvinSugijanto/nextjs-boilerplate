...
export async function GET(req, { params }) {
  ...
  try {
    const record = await pb.collection(collection).getOne(params.id, {
      headers: {
...
export async function PUT(req, { params }) {
  ...
  if (contentType.includes('multipart/form-data')) {
    ...
    try {
      const record = await pb.collection(collection).update(params.id, data, {
        headers: {
          Authorization: authorization
        }
      })
...
export async function DELETE(req, { params }) {
  ...
  try {
    const record = await pb.collection(collection).delete(params.id, {
      headers: {
        Authorization: authorization
      }
    })
